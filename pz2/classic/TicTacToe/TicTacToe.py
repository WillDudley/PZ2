...
class TicTacToeEnv(Env):
    ...
    def step(self, actions):
        ...
        for agent, action in actions.items():
            if action is not None:
                ...
                # play turn
                self.board.play_turn(self.agents.index(agent), action)

                observation[agent] = self._observe(agent)
        ...
        # Find out who plays in the next turn
        next_active_agents = {agent: False for agent in self.agents}
        for agent in self.agents:
            if actions[agent] is None:
                next_active_agents[agent] = True

        return observation, rewards, terminations, {a: False for a in self.agents}, next_active_agents, {}
    ...
    def reset(self, seed=None, return_info=False, options=None):
        ...
        observation = {agent: self._observe(agent) for agent in self.agents}
